"""Specialist agent that generates and executes data visualization code, saving charts as PNG files."""
...
from src.prompts.system import VIZ_AGENT_SYSTEM
...
def _call_llm(prompt: str) -> str:
    ...
    response = _client.responses.create(
        ...
        input=[
            {"role": "system", "content": VIZ_AGENT_SYSTEM},
...
def run_viz_agent(
    ...
    """Generate and execute data visualization code, collecting any saved PNG/JPG charts."""
    logger.info("specialist_start", agent="viz", subtask_id=subtask.id)
    ...
    try:
        ...
        logger.info("specialist_complete", agent="viz", charts_generated=len(charts))
        ...
        logger.error("specialist_error", agent="viz", error=str(exc))
        return SubtaskResult(subtask_id=subtask.id, agent_type=AgentType.VIZ, error=str(exc), success=False)
